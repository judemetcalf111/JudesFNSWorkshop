...
centerangle(t) = exp.( im .* k .* t)
centerangle90(t) = exp.( im .* ( k .* t .+ (π/2)))
...
for file in csv_files
    ...
    for (index,xvalue) in enumerate(x)
        xshift[index] = xvalue - real(center(timestamp[index]))
    end
    for (index,yvalue) in enumerate(y)
        yshift[index] = yvalue - imag(center(timestamp[index]))
    end

    for (index,timevalue) in enumerate(timestamp)
        xpos[index] = (xshift[index] * real(centerangle(timevalue))) + (yshift[index] * imag(centerangle(timevalue)) )
        ypos[index] = (xshift[index] * real(centerangle90(timevalue))) + (yshift[index] * imag(centerangle90(timevalue)) )
    end
    ...
    hm = CairoMakie.heatmap!(ax, xbins, ybins, log1p.(heatdatashift)', colormap=:viridis)
    Colorbar(figshift[1,2], hm)
    figshift


    # Build output path with same base filename but PNG extension
    outname = joinpath(output_dir, splitext(basename(file))[1] * "-MovingFrame" * ".png")
    ...
    hm = CairoMakie.heatmap!(ax, xbins, ybins, log1p.(heatdatapos)', colormap=:viridis)
    Colorbar(figpos[1,2], hm)

    # Build output path with same base filename but PNG extension
    outname = joinpath(output_dir, splitext(basename(file))[1] * "-CoordinateFrame" * ".png")

    # Save figure
    save(outname, figpos)

    println("Saved heatmap for $splitext(basename(file))[1]")
end
