...
begin # Generate a distribution to sample
    ...
    center(t) = 0*(xmax ./ 2) .* exp.( im * k * t) # Note the vanishing
	
    wells(t) = [MvNormal([real(center(t)), imag(center(t))], 4*I(2))]
...
begin
	...
	trail!(ax, xy; n_points=200, colormap=:Purples, color=1:100) # You could go fancy by also adding `colormap=:turbo, color=1:100`
	TimeseriesMakie.heatmap!(ax, xs, xs, X, colormap=seethrough(:turbo), colorrange=(0, 0.25/4))
	TimeseriesMakie.scatter!(ax, xy_last; markersize=15, color=:red)
	hidedecorations!(ax)
	hidespines!(ax)
end
...
begin
	...
	global β_value = 0.2
	global γ_value = 0.15
	fullpath = path * "/data/exp_raw/longanalysis/a=$(α_value)/b=$(β_value)/k=$(k)/"
	mkpath(fullpath)

	timespan = 500.0
	seeds = [25]#[27,5,3201,4325,2835,3746,2343,29,145,7467] 
	for seedvalue in seeds
		...
		xy[] = [Point2f([NaN, NaN])]
		file = record(fig, path * "/data/sims/" * filename * ".mp4", range(1, length(sol), step=1000); dpi = 300,
		        framerate = 48) do i
			t = sol.t[i]
		    X[] = G(t).(Xs)
