...
csv_files = filter(f -> startswith(f,input_dir * "/tfnsBrownian") && contains(f,"Plateau") && endswith(f, ".csv"), readdir(input_dir; join=true))
...
for file in csv_files
    ...
    xshift,yshift = xshift[1:lengthofdata ÷ 2], yshift[1:lengthofdata ÷ 2]   ## Anomalous Energy kick accounting
    ...
    filepath = "/BCPlateau_a=$(aval)/b=$(bval)/k=$(kval)/" 
    ...
    #     title = "Plateau Radial Density: α = $(aval), β = $(bval), k = $(kval)",
    #     minorgrid = true,
    #     lw=2.5,
    #     c=:blue)

    # Plots.vline!([mean(r)], c = "red", label = "mean")
    # Plots.vline!([5], c = "green", label = "shelf radius")
    # Plots.plot!(bin_midpoints,fitline, c = :purple, label = "exp($(round(Coeffs[2], digits=3))×r)")
    # @infiltrate
    println("$(aval) and $(mean(r))")#            For when we get a fit...
    #                                             and $(round(Coeffs[2], digits=3))")
    # display(XtragramNEW)
